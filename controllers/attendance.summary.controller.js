...
const attendancy_summary = async(req, res) => {
    ...
    for (let i = 0; i < data1.length; i++) {
        ...
        let querybraketime = `select total_brake_time from brake_system where brake_date ="${data1[i].check_date}" and basic_info_id = ${req.session.emp_id};`
    ...
    res.render("attendance", { data1, total_brake, total_workhours, all_work_hours, all_stafing_hours, totalratio, "first_name": userInfo[0].first_name, "profilePhoto": profilePhoto[0].profile_photo })
}
